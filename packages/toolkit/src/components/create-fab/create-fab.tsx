...
export function CreateFab({ onClick, label = "Erstellen", className }: CreateFabProps) {
  return (
    <button
      ...
      title={label}
      className={cn(
        "fixed right-6 bottom-[calc(1.5rem+env(safe-area-inset-bottom))] z-30 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg transition-all hover:scale-105 hover:shadow-xl active:scale-95",
        className,
      )}
    >
