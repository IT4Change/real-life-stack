...
| Eintragstyp-FAB mit Typ-Auswahl | `App.tsx` (FAB + DropdownMenu), `hooks/useComposer.ts`, `hooks/useScrollFab.ts` | Floating-Action-Button oeffnet Dropdown zur Auswahl des Eintragstyps (Post/Event/Quest/Crowdfunding), startet den passenden Composer; FAB blendet beim Scrollen weicher aus. | Teilweise. Toolkit hat `create-fab` + `feed-composer-trigger`, aber die **Typ-Vorauswahl per Dropdown** und das Scroll-Fade sind hier eigenstaendig. | Niedrig bis mittel. Kleiner Baustein, gut isolierbar. Hinweis: Prototyp-`POST_TYPES` sind inkonsistent (siehe Abschnitt ,Drift). | Ja |
...
| Toast-System | `components/ui/use-toast.ts`, `toaster.tsx`, `toast.tsx` | Radix-basiertes Toast-System. | Wahrscheinlich vorhanden. Toolkit hat eigene Primitives; nur abgleichen. | Niedrig. Vermutlich Duplikat; nicht separat uebernehmen. | Nein |

## Nicht uebernehmen (Platzhalter/Demo)

Aus dem Code eindeutig als Stub erkennbar, kein Uebernahmewert:

- ,Feature nicht implementiert-Toasts in `Navbar.tsx` (User-Menue), `Sidebar.tsx` (komplettes Seitenmenue ist Demo), `PostCard.tsx` (Teilen/Kommentare-Buttons), `MainContent.tsx` (Feed-Filter), sowie in den Profil-Aktionen (Beitreten/Spenden/Quest).
...
- **Inkonsistente Eintragstypen:** `App.tsx`/`useComposer.ts` nutzen `POST_TYPES` = Post/Event/Quest/Crowdfunding, `SmartPostWidget.tsx` nutzt ein **anderes** Set Post/Veranstaltung/Projekt/Anzeige, und `types/index.ts` `Post.type` = person/event/project/offer/quest. Bei Uebernahme zwingend an die RLS-Item-Typen ([item-types.md](./item-types.md)) angleichen.
