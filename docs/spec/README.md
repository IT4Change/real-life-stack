...
- **Stub-Conformance-Tests** pro Modul-Spec: ein Test prüft, dass der versprochene Vertrag (z.B. `MapAdapter` aus `modules/map.md`) im Code so existiert. Wenn jemand eine Adapter-Methode entfernt, bricht der Test mit Verweis auf die Spec-Stelle.
  - ✅ **MapAdapter** — `packages/toolkit/tests/spec-conformance/map-adapter.test.ts` prüft das Interface, eine strukturelle Stub-Implementation und `LeafletMapAdapter` gegen die Method-Liste aus `modules/map.md`.
  - Calendar, Feed, Kanban folgen als analoge Tests (kein Adapter-Pattern wie Map, aber definierte Item-Filter und Layout-Verträge).
