...
2. **Scalar lowercase.** Typ-Identifier sind kleingeschriebene Scalars (`person`, `event`, `task`, `place`). Sie matchen den `@context`-Vokabularnamen, wo ein Vokabular existiert (`event` → `event/v1`, `place` → `place/v1`, `task` → `task/v1`, `person` → `person/v1`). Diese Zuordnung ist reine Namenskorrespondenz und steuert nicht die Modul-Aktivierung; welche Vokabulare tatsächlich aktiv werden, regelt Regel 3 (`deriveContext`), feldbasiert für `event/v1`/`place/v1` (`data.start` bzw. `data.position`) und per `type` nur für `task/v1`/`person/v1`.
