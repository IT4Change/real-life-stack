...
Der Feed ist als einziger Eintrag der Tabelle **keine feldaktivierte Sicht**, sondern eine aggregierende (oben, „Verhältnis zwischen Schema- und Feldfiltern", zusammen mit der Suche): er beantwortet „was ist hier neu", nicht „welches Feld kann ich rendern". Deshalb MUSS er alle Items des Scopes zeigen, die für sich stehen, und darf sie nicht nach Feldern oder Typen auswählen. Nicht für sich stehen ausschließlich die System-Typen (`comment`, `reaction`, `relation` — sie gehören zu einem anderen Item) und `feature` als Geometrie-Marker; das Prädikat dafür ist `isStandaloneItemType` in `data-interface`. Es benennt bewusst die Eigenschaft des Items, nicht seine Darstellung: ob eine Fläche daraus eine Karte, eine Zeile oder einen Kartenpin macht, entscheidet sie selbst. Ein Feed, der Typen aufzählt, verliert jeden neuen Typ lautlos — er hat kein Register zu führen.
