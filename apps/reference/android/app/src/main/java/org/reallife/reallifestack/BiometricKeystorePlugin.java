...
/**
 * Stores the WoT identity passphrase encrypted with a biometric-gated
 * AES key in the Android Keystore, so the user can unlock with their
 * fingerprint/face instead of re-typing the passphrase.
 *
 * The passphrase never leaves the device: it is AES-GCM encrypted with a
 * key that requires user authentication, and only the ciphertext + IV are
 * persisted in app-private SharedPreferences. The JS side talks to this via
 * registerPlugin('BiometricKeystore') — keep the plugin name in sync.
 */
@CapacitorPlugin(name = "BiometricKeystore")
public class BiometricKeystorePlugin extends Plugin {

    private static final String KEYSTORE_ALIAS = "rls_biometric_key";
    private static final String PREFS_NAME = "rls_biometric_prefs";
